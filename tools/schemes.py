...
def bubble(x, y, w, h, s, who='you', size=14):
    """Реплика чата: справа — твоя (фиолетовая), слева — ответ ИИ (белая)."""
    fill, col = (C['ai'], '#fff') if who == 'you' else ('#fff', INK)
    b = rect(x, y, w, h, fill, INK, 2, 14)
    for i, ln in enumerate(s):
        b += text(x + 14, y + 26 + i * 20, ln, size, 700 if who == 'you' else 600, col)
    return b


def robot(x, y, sc=1.0):
    g = rect(x, y + 10 * sc, 70 * sc, 54 * sc, C['ai'], INK, 2, 12)
    g += line(x + 35 * sc, y, x + 35 * sc, y + 10 * sc)
    g += f'<circle cx="{x + 35 * sc}" cy="{y}" r="{5 * sc}" fill="{YEL}" stroke="{INK}" stroke-width="2"/>'
    g += f'<circle cx="{x + 22 * sc}" cy="{y + 34 * sc}" r="{6 * sc}" fill="#fff"/><circle cx="{x + 48 * sc}" cy="{y + 34 * sc}" r="{6 * sc}" fill="#fff"/>'
    g += line(x + 24 * sc, y + 50 * sc, x + 46 * sc, y + 50 * sc, '#fff', 3)
    return g


def a1():
    b = window(30, 30, 520, 330, 'Чат с ИИ-помощником', C['ai'])
    b += bubble(230, 60, 300, 46, ['Почему небо голубое?'], 'you')
    b += bubble(50, 120, 420, 96, ['Солнечный свет состоит из разных цветов.', 'Воздух сильнее всего рассеивает голубой —', 'поэтому небо кажется голубым.'], 'ai')
    b += bubble(230, 232, 300, 46, ['А сколько лет моей кошке?'], 'you')
    b += bubble(50, 292, 420, 52, ['Этого я знать не могу: я тебя не знаю.'], 'ai')
    b += robot(600, 70, 1.2)
    b += callout(1, 600, 200, 'Отвечает словами')
    b += callout(2, 600, 250, 'Не человек: не думает')
    b += text(622, 275, 'и не чувствует', 13, 600, MUTED)
    b += callout(3, 600, 320, 'Не знает тебя лично')
    b += text(30, 400, 'ИИ прочитал огромное количество текстов и подбирает подходящие слова. Иногда — неправильные.', 14, 700, MUTED)
    return svg(840, 430, b)


def a2():
    b = text(30, 34, 'Один и тот же вопрос — разные ответы', 17, 800)
    b += rect(30, 50, 380, 330, '#fff', BAD, 3, 14)
    b += text(50, 82, '✕ Слишком общая просьба', 15, 800, BAD)
    b += bubble(50, 96, 340, 42, ['Расскажи про Марс'], 'you', 13)
    b += rect(50, 150, 340, 210, '#f7f7fb', '#dfe3ee', 2, 10)
    for i, ln in enumerate(['Марс — четвёртая планета от Солнца.', 'Он имеет красноватый оттенок из-за', 'оксида железа в грунте. Атмосфера', 'разрежена и состоит в основном из', 'углекислого газа. Период обращения…']):
        b += text(66, 180 + i * 22, ln, 13, 600, MUTED)
    b += text(66, 330, 'Длинно, непонятно, не для доклада', 13, 800, BAD)
    b += rect(430, 50, 380, 330, '#fff', OK, 3, 14)
    b += text(450, 82, '✓ Точная просьба', 15, 800, OK)
    b += bubble(450, 96, 340, 76, ['Я в 4 классе. Дай 5 фактов о Марсе', 'для доклада, списком, простыми', 'словами, по одной строке'], 'you', 13)
    b += rect(450, 184, 340, 176, '#f4fbf6', '#cfe9d8', 2, 10)
    for i, ln in enumerate(['1. Марс — четвёртая планета от Солнца.', '2. Его называют Красной планетой.', '3. Там есть самая высокая гора.', '4. Сутки почти как на Земле.', '5. Воды в виде льда — на полюсах.']):
        b += text(466, 212 + i * 24, ln, 13, 700)
    b += text(30, 410, 'Скажи: кто ты, зачем нужно, сколько пунктов и каким должен быть ответ.', 14, 700, MUTED)
    return svg(840, 430, b)


def a3():
    b = window(30, 30, 460, 250, 'Чат с ИИ-помощником', C['ai'])
    b += bubble(60, 60, 400, 42, ['Кто написал «Му-му»?'], 'you', 13)
    b += bubble(60, 116, 400, 46, ['«Му-му» написал Лев Толстой.'], 'ai', 13)
    b += rect(60, 178, 400, 74, '#fdeeee', BAD, 2, 10)
    b += text(80, 206, '⚠ Это неправда: написал Тургенев.', 14, 800, BAD)
    b += text(80, 230, 'ИИ ошибся, но говорил уверенно.', 13, 600, MUTED)
    b += callout(1, 540, 90, 'Проверь в учебнике')
    b += callout(2, 540, 150, 'Поищи на надёжном сайте')
    b += callout(3, 540, 210, 'Спроси взрослого')
    b += rect(520, 250, 290, 120, '#fff', INK, 2, 12)
    b += text(540, 282, 'Особенно проверяй:', 14, 800)
    for i, ln in enumerate(['даты и числа', 'имена и фамилии', 'правила и формулы']):
        b += text(556, 308 + i * 22, '• ' + ln, 13, 700, MUTED)
    b += text(30, 320, 'Уверенный тон — не доказательство.', 15, 800)
    b += text(30, 348, 'Если ИИ спорит и настаивает, он всё равно', 13, 700, MUTED)
    b += text(30, 368, 'может ошибаться. Верь проверке, а не тону.', 13, 700, MUTED)
    b += text(30, 412, 'Выдуманный ответ, который звучит правдоподобно, — обычное дело для ИИ.', 14, 700, MUTED)
    return svg(840, 430, b)


def a4():
    b = text(30, 34, 'Что можно и что нельзя отправлять ИИ', 17, 800)
    b += rect(30, 50, 380, 330, '#fdeeee', BAD, 3, 14)
    b += text(50, 84, '✕ Никогда не отправляй', 15, 800, BAD)
    for i, ln in enumerate(['пароли и коды из СМС', 'домашний адрес и телефон', 'школу, класс, когда ты один дома', 'фото документов и банковских карт', 'чужие фото и чужие секреты']):
        y = 110 + i * 48
        b += rect(50, y, 340, 38, '#fff', BAD, 2, 10)
        b += text(68, y + 25, '✕', 15, 900, BAD) + text(92, y + 25, ln, 13, 700)
    b += rect(430, 50, 380, 330, '#f4fbf6', OK, 3, 14)
    b += text(450, 84, '✓ Можно спокойно', 15, 800, OK)
    for i, ln in enumerate(['вопросы по учёбе', 'задачу из учебника', 'придуманную историю', 'просьбу объяснить непонятное', 'проверить свой текст на ошибки']):
        y = 110 + i * 48
        b += rect(450, y, 340, 38, '#fff', OK, 2, 10)
        b += text(468, y + 25, '✓', 15, 900, OK) + text(492, y + 25, ln, 13, 700)
    b += text(30, 410, 'Всё, что ты отправил, уходит на чужой компьютер и может там сохраниться.', 14, 800, INK)
    return svg(840, 430, b)


def a5():
    b = text(30, 34, 'ИИ помогает учиться, а не учится за тебя', 17, 800)
    b += rect(30, 54, 380, 300, '#fff', BAD, 3, 14)
    b += bubble(50, 80, 340, 64, ['Напиши за меня сочинение', 'про осень, я сдам'], 'you', 13)
    b += rect(50, 160, 340, 100, '#fdeeee', BAD, 2, 10)
    b += text(70, 190, 'Учитель просит твои мысли,', 13, 700)
    b += text(70, 212, 'а не чужой текст.', 13, 700)
    b += text(70, 240, 'Списал — не научился.', 13, 800, BAD)
    b += text(70, 292, '😕 На контрольной ИИ рядом', 14, 800, BAD)
    b += text(70, 316, 'не будет.', 14, 800, BAD)
    b += rect(430, 54, 380, 300, '#fff', OK, 3, 14)
    b += bubble(450, 80, 340, 64, ['Не решай за меня. Объясни,', 'как решать такие задачи'], 'you', 13)
    b += rect(450, 160, 340, 100, '#f4fbf6', OK, 2, 10)
    b += text(470, 190, 'Объясни, приведи пример,', 13, 700)
    b += text(470, 212, 'найди мою ошибку,', 13, 700)
    b += text(470, 234, 'придумай задачи для тренировки.', 13, 700)
    b += text(470, 292, '⭐ Понял сам — умеешь сам.', 14, 800, OK)
    b += text(30, 390, 'Пользовался ИИ для домашки — скажи об этом взрослым или учителю. Это честно.', 14, 700, MUTED)
    return svg(840, 430, b)


def a6():
    b = text(30, 34, 'Рутину — помощнику, интересное — себе', 17, 800)
    b += rect(30, 56, 300, 300, '#f3f4f8', '#dfe3ee', 2, 14)
    b += text(50, 88, 'Скучное и повторяющееся', 14, 800, MUTED)
    for i, ln in enumerate(['переписать начисто', 'найти опечатки', 'составить план', 'придумать 10 идей', 'сделать таблицу из списка']):
        y = 108 + i * 46
        b += rect(50, y, 260, 36, '#fff', '#dfe3ee', 2, 9)
        b += text(66, y + 24, '• ' + ln, 13, 600)
    b += robot(378, 150, 1.1)
    b += arrow(350, 190, 372, 190)
    b += arrow(462, 190, 496, 190)
    b += rect(510, 56, 300, 300, '#f4fbf6', OK, 3, 14)
    b += text(530, 88, 'Освободилось время', 14, 800, OK)
    b += text(530, 130, '⏱', 30, 400)
    b += text(570, 132, 'на то, что интересно', 15, 800)
    for i, ln in enumerate(['разобраться, как это работает', 'придумать своё', 'погулять и поиграть', 'дочитать книгу']):
        b += text(530, 176 + i * 34, '✓ ' + ln, 13, 700)
    b += text(530, 330, 'Это и называется автоматизацией', 13, 800, OK)
    b += text(30, 392, 'Помощник не устаёт и не обижается: можно переспрашивать сколько угодно, пока не станет понятно.', 14, 700, MUTED)
    b += text(30, 414, 'Решение всё равно за тобой: ты проверяешь и выбираешь, что оставить.', 14, 700, MUTED)
    return svg(840, 440, b)
